...
Structure detection now resolves the real `StructureStart` from nearby chunk references instead of asking only whether the player is already inside the structure. A small exit buffer is then applied to the real bounding box: structures up to 16 blocks wide/deep receive 2 blocks of horizontal padding on each side, larger structures receive 1 block, and every mapped structure receives 2 blocks of vertical padding. This keeps small structures such as Ruined Portals from rapidly entering and leaving the music zone at their exact edge.
...
A detecção de estruturas agora resolve o `StructureStart` real pelas referências dos chunks próximos em vez de depender de o jogador já estar dentro da estrutura. Depois aplica uma pequena margem à bounding box real: estruturas com até 16 blocos de largura/profundidade recebem 2 blocos extras em cada lado horizontal, estruturas maiores recebem 1 bloco, e todas recebem 2 blocos extras na vertical. Isso evita que estruturas pequenas como Ruined Portals entrem e saiam da zona musical repetidamente nas bordas.
